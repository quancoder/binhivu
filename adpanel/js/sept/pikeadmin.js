        ...
        if(!$(this).hasClass("subdrop")) {
          ...
          // open our new menu and add the open class
            $(this).next("ul").slideDown(350, function(){
				var th = $('#sidebar-menu').outerHeight();
				var hbar = $('#main .headerbar').outerHeight();
                var hmain = $('.container-fluid').outerHeight();
				var wh = $(window).height();
				if(th > hmain)
				{
                    
					$("body").css({'min-height':(hbar + th) + 'px'});
...
function set_min_height()
{
    ...
    var hbar = $('#main .headerbar').outerHeight();
    var mainbar = $('.main-sidebar #sidebar-menu').outerHeight();
    var wh = $(window).height();
    var hconten = $(".content-page").outerHeight();
    if(hconten + hf > wh)
    {
        $("body").css({'min-height':(hconten + hf) + 'px'});
    }
    else
    {
        ...
        if(mainbar + hbar > wh)
        {
            $("body").css({'min-height': (mainbar + (hbar * 2)) + 'px'});
        }
