...
class JWT
{
    public static function ApiToken($domain, $key)
    {
        $algo = 'HS512';
        $payLoadData = array(
            'domain' => $domain,
        ...
        return self::Encode($payLoadData, $algo, $key);
    }

    public static function Encode($payloadData, $algo = 'HS256', $key, $type = 'JWT')
    {
        $algo = strtoupper($algo);
        $header = array('alg' => $algo, 'typ' => $type);
        $payloadData = self::_base64Encode(json_encode($payloadData));
    ...
    private static function _verify($key, $header, $payload, $signature, $numSecondExpire)
    {
        ...
        */
        ...
        return (
        self::_hash_equals(self::_sign($signWith, $key, $algorithm), $signature)
        );
    }
