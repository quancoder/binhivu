...
!function (a, b) {
    ...
    function () {
        var e;
        e = jQuery.noConflict(!0), window.BizwebFormEmbed = {
            $: e,
            Script: {},
        }
    }.call(this),
    ...
    function () {
        var $ = BizwebFormEmbed.$;
        BizwebFormEmbed.Script = function () {
            var n = this;
            n.fjs = $('#bizweb-form-jssdk').first();
            n.shop = n.fjs.attr('data-shop');
            n.domain = n.fjs.attr('data-domain');
            n.array = new Array();

            n.checkForm = function ($item) {
                var formId = $item.attr('data-form-id');
                var check = n.filter(formId);
                if (check) {
                    var id = 'bizweb-form-' + formId;
                    if ($('iframe#' + id).length) {
                        return false;
                    }
                    var iframe = '<iframe id="' + id + '" width="100%" height="0" style="border:none;" src="' + n.domain + '/bizwebformview/?store=' + n.shop + '&formId=' + formId + '"></iframe>';
                    $item.html(iframe);
                }
            }
            n.filter = function (id) {
            ...
            n.resize = function ($item) {
                var formId = $item.attr('data-form-id');
                var item = n.getItem(formId);
                if (item != null) {
                    var w = $(window).width();
                    if (w > 720) {
                        $item.css('width', item.widthD + '%');
                    }
                    else {
                        $item.css('width', item.widthM + '%');
                    }
            ...
            n.hienThi = function (event) {
                var data = event.data;
                //console.log(data);

                if (data.type !== 'bizweb-form') {
                    return;
                }

                var id = 'bizweb-form-' + data.f;
                var $form = $('#' + id).first();
                $form.css('height', data.h);
            }
            n.start = function () {
                var $form = $("div[data-app='bizweb_form']");
                if ($form.length) {
                    var url = n.domain + '/bizwebformview/checkform';
                    var dataJson = {
                        store: n.shop,
                    };
                    $.ajax({
        ...
        $(function () {
            var bizwebForm = new BizwebFormEmbed.Script();
            bizwebForm.start();
            if (window.addEventListener) {
                window.addEventListener("message", bizwebForm.hienThi, false);
            }
            else {
                window.attachEvent("onmessage", bizwebForm.hienThi);
